...
function SortableJobCard({
  ...
  }); //registers a card as a draggable item
...
export default function KanbanBoard({ board, userId }: KanbanBoardProps) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    //Check if the job is dropped over a column or another job
    ...
    if (targetColumn) {
      ...
      const jobsInTarget =
        targetColumn.jobApplications
          ...
          .sort((a, b) => a.order - b.order) || [];
      newOrder = jobsInTarget.length; // If dropped in an empty column, place it at the end
    } else if (targetJob) {
      ...
      if (targetIndexInFiltered !== -1) {
        if (sourceColumn._id === targetColumnId) {
          if (sourceIndex < targetIndexInFiltered) {
            newOrder = targetIndexInFiltered + 1; // Moving down in the same column
          } else {
            newOrder = targetIndexInFiltered; // Moving up in the same column
          }
        } else {
          newOrder = allJobsInTargetFiltered.length; // Moving to a different column
        }
      } else {
        return; // If dropped at the end of the column
      }

      if (!targetColumnId) return;

      moveJob(activeId, targetColumnId, newOrder);
    }
  }
