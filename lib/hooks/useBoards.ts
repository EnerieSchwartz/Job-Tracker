...
export function useBoard(initialBoard?: Board | null) {
  ...
  async function moveJob(
    ...
    // console.log(`Moving job ${jobApplicationId} to column ${newColumnId} at order ${newOrder}`,
    // );
 ...
 setColumns((prev) => {
      ...
      if(targetColumnIndex !== -1) {
        ...
        updatedJobs.splice(newOrder, 0, {...jobToMove, columnId: newColumnId, order: newOrder*100});
    ...
    try{
      const result = await updateJobApplication(jobApplicationId, {columnId: newColumnId, order: newOrder});
